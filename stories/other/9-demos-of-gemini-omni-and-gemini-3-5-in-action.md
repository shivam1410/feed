---
title: "9 demos of Gemini Omni and Gemini 3.5 in action"
category: "Other"
source: "DeepMind"
url: "https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-omni-3-5-videos/"
authors: ["Zahra Thompson"]
date: "Fri, 29 May 2026 17:30:00 +0000"
score: 30
guid: "https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-omni-3-5-videos/"
image: "https://storage.googleapis.com/gweb-uniblog-publish-prod/images/Gemini_Omni_and_Gemini_3.5_hero.max-600x600.format-webp.webp"
generated: "2026-09-08T19:05:47+05:30"
---

Google demonstrated Gemini Omni and Gemini 3.5 capabilities through nine video demonstrations announced at Google I/O 2026. The videos showcase the model features and performance across various tasks. These public demonstrations are meant to illustrate capabilities to developers and users evaluating the models for adoption.
